...
import type { Quote, InvoiceClient, Product, CreateQuoteInput, InvoiceSettings } from '../../types'
import { quoteFormSchema, type QuoteFormData, type LineItemFormData } from '../schemas'
...
export const QuoteForm: FC<QuoteFormProps> = ({
  ...
  const initialLineItems: LineItemFormData[] = quote?.lineItems?.map((li) => ({
    id: li.id,
  ...
  const {
    ...
    defaultValues: quote
      ...
      : {
          ...
          validUntil: calculateValidUntil(settings?.defaultQuoteValidity || 30),
          notes: settings?.quoteNotesTemplate || '',
          footer: settings?.quoteFooterTemplate || '',
          lineItems: [],
        },
  })
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              id="subject"
              placeholder="Ex: Proposition commerciale - Refonte site web"
              {...register('subject')}
            />
          </div>
          <div className="space-y-2">
            ...
            <Textarea
              id="introduction"
              placeholder="Suite a notre entretien du..."
              rows={3}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              id="terms"
              placeholder="Conditions de validite, delais, etc."
              rows={2}
              {...register('terms')}
            />
          </div>
